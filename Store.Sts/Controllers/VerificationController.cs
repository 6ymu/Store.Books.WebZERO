...
namespace Store.Sts.Controllers
{
    ...
    public class VerificationController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] LoginViewModel model, CancellationToken token)
        {
            ...
            var body = GetBody(verifyToken, resendToken);
            _logger.LogInformation($"Sending sms ok, token: {verifyToken}");
...
/*
        private async Task<ApplicationUser> GetUser(string phoneNumber)
        {
            ...
            var user = await _userManager.Users.SingleOrDefaultAsync(x => x.PhoneNumber == normalizedPhoneNumber)
                       ?? new ApplicationUser
                       {
                           UserName = phoneNumber,
                           PhoneNumber = phoneNumber,
                           SecurityStamp = ("secret".GetHash(SecurityExtension.HashType.SHA512)
                                + phoneNumber.GetHash(SecurityExtension.HashType.SHA256))
